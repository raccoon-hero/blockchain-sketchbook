...
class PKO_Blockchain:
    ...
    def add_block(self, miner_address):
        ...
        # COINBASE-ТРАНЗАКЦІЯ (10 per block)
        coinbase_transaction = PKO_Transaction("Network", miner_address, 10 + total_fees)
    ...
    def get_chain(self):
        return [block.to_dict() if isinstance(block, PKO_Block) else block for block in self.chain]
    ...
    def block_hash(self, block):
        ...
        block_obj.timestamp = block.timestamp
        block_obj.nonce = block.nonce
        return block_obj.calculate_hash()
    ...
    def resolve_conflicts(self):
        ...
        for node in neighbours:
            ...
            if response.status_code == 200:
                ...
                chain = [self.dict_to_block(block) for block in chain]
...
def new_transaction():
    ...
    if not all(k in values for k in required):
        return 'Not all data received', 400
...
def consensus():
    ...
    if replaced:
        response = {
            'message': 'Our chain was replaced by a new one',
            'new_chain': [block.to_dict() for block in blockchain.chain]
        }
    else:
        response = {
            'message': 'Our chain is the main one',
